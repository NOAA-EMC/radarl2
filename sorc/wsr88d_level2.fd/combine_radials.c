...
/* $Author: cm $ */
/* $Locker:  $ */
/* $Date: 2011-09-15 09:32:18-05 $ */
/* $Id: combine_radials.c,v 1.2 2011-09-15 09:32:18-05 cm Exp $ */
/* $Revision:  */
/* $State: */
...
static float Get_z (int dbz, int rind);
...
int CR_combine_radials (char *input, char **output, int *length) {
    ...
    *length = 0;
	...
	if (len > 0 ) { 
           ...
           if (bh->n_dop_bins > BASEDATA_VEL_SIZE)
               bh->n_dop_bins = BASEDATA_VEL_SIZE;
...
static int Setup_and_combine (char *rad1, char *rad2,
    ...
    *length = 0;
    ...
    NOISE = bh1->horiz_noise;
    SYSCAL = bh1->calib_const - NOISE;
	...
	LE_send_msg (0, 
	    "SYSCAL %f, ATMOS %f, NOISE %f, THZ %f, THV %f, THW %f, VNYQ %f",
			SYSCAL, ATMOS, NOISE, THZ, THV, THW, VNYQ);
...
/*
    ...
    *length = out_size;
	...
	if (outrad->elev_num == ORPGVCP_get_num_elevations (outrad->vcp_num ))
...
static void Combine_radials (int ns, int nd,
    ...
    if (REF_GATE_1000)
	Get_z (ns, GP_INIT_LONG_GATE);
    else
	Get_z (ns, GP_INIT_SHORT_GATE);

    TZ = exp10f ((NOISE + THZ) * .1f);
    TV = exp10f ((NOISE + THV) * .1f);
    TW = exp10f ((NOISE + THW) * .1f);
	...
	for (j = 0; j < 4; j++) {
		...
		ZNUM += Get_z (z1[s], s);		/* Accumulate z */
		if (z2 != NULL) {
		    Z1NUM = Get_z (dz1[s], s);
		    Z2NUM = Get_z (dz2[s], s);
		    ZNUM += Get_z (z2[s], s);		/* Accumulate z */
		}
	    }
...
static void Combine_azi (int ns, int nd,
	...
	ZNUM = (Get_z (z1[i], i) + Get_z (z2[i], i)) * .5f;
...
static void Combine_range (int ns, Moment_t *z1, Moment_t *zr) {
	...
	ZNUM += Get_z (z1[i], i);		/* Accumulate z */
	if ((i % 4) < 3)
	    continue;

	/* Censor recombined reflectivity */
	m = i >> 2;
/* Sould divide by 4.0 because ZNUM is total sum   Guan 07/18/2012 */
	if (ZNUM * Range_corr_l[m]/4.0 < TZ)	/* Below the SNR threshold */
	    zr[m] = 0;
	else			/* Re-quantize recombined reflectivity */
...
static float Get_z (int dbz, int rind) {
    ...
    static float guessed_power, *range_cor;
    int ns, i;

    if (rind >= 0) {
	if (dbz <= 0)	/* Fill in censored gate with artificial value */
	    return (guessed_power / range_cor[rind]);
	else {
	    if (dbz > 255)
		dbz = 255;
	    return (z_table[dbz]);
    ...
    guessed_power = noise_assumption * exp10f ((NOISE + THZ) * .1f);
