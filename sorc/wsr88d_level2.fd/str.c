 ...
 * $Author: jing $
 * $Locker:  $
 * $Date: 2007/02/13 21:47:50 $
 * $Id: str.c,v 1.10 2007/02/13 21:47:50 jing Exp $
 * $Revision: 1.10 $
 * $State: Exp $
 */
...
static char *Str_expand (char *str, int length);
...
char *STR_append (char *dest_str, const char *src_str, int size) {
    int nb = 0;
    if (dest_str != NULL)
	nb = ((Str_hd_t *)(dest_str - sizeof (Str_hd_t)))->n_bytes;
    dest_str = Str_expand (dest_str, nb + size);
    if (src_str != NULL)
	memcpy (dest_str + nb, src_str, size);
    ((Str_hd_t *)(dest_str - sizeof (Str_hd_t)))->n_bytes = nb + size;
    return (dest_str);
}
...
int STR_size (char *str) {
    if (str == NULL)
	return (0);
    return (((Str_hd_t *)(str - sizeof (Str_hd_t)))->n_bytes);
}
...
/***********************************************************************
    ...
    Expands the buffer of "str" with additional space.
